...
// Serve the frontend HTML/CSS/JS pages (login, dashboard, markets, etc.)
// directly from the repo root, so visiting the site's URLs works.
...
const ENV = {
  ...
  FINNHUB_STOCK_API_KEY: process.env.FINNHUB_STOCK_API_KEY,        // 🔒 set on host
...
const stockService = {
  async getMarketData(ticker) {
    ...
    try {
      ...
      // Finnhub returns c: 0 (and no other fields) for an invalid/unknown symbol.
      if (!quote || (quote.c === 0 && quote.pc === 0)) {
        return { symbol: ticker.toUpperCase(), currentPrice: null, status: 'Symbol not found' };
      }
      ...
      return { symbol: ticker.toUpperCase(), currentPrice: null, status: 'Live data fetch failed' };
    }
  },
...
const stripeService = {
  ...
  async createCoreSubscriptionCheckout(userId, billingCycle = 'monthly') {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: 'https://analyst-iq.onrender.com/analystiq-dashboard.html?tier=core',
      cancel_url: 'https://analyst-iq.onrender.com/analystiq-dashboard.html?cancelled=1',
    });
    return session.url;
  },
  ...
  async createMasterKeyCheckout(userId) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: 'https://analyst-iq.onrender.com/analystiq-dashboard.html?tier=master',
      cancel_url: 'https://analyst-iq.onrender.com/analystiq-dashboard.html?cancelled=1',
    });
    return session.url;
  },
  ...
  async createPDFCheckout(userId) {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: 'https://analyst-iq.onrender.com/analystiq-book.html?purchased=1',
      cancel_url: 'https://analyst-iq.onrender.com/analystiq-book.html?cancelled=1',
    });
    return session.url;
  },
