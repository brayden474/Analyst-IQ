...
async function fetchQuoteForSymbol(symbol) {
  ...
  const quote = await quoteRes.json();
  const profileRes = await fetch(`https://finnhub.io/api/v1/stock/metric?symbol=${symbol}&metric=all&token=${apiKey}`);
  const metrics = await profileRes.json();
  ...
  return {
    ...
    high52w: metrics.metric?.['52WeekHigh'] ?? null,
    low52w: metrics.metric?.['52WeekLow'] ?? null,
...
async function refreshMarketCache() {
  if (!ENV.FINNHUB_STOCK_API_KEY) return; // don't burn a cycle with no key configured
  try {
    const results = await Promise.allSettled(TRACKED_SYMBOLS.map(fetchQuoteForSymbol));
...
refreshMarketCache();
setInterval(refreshMarketCache, MARKET_REFRESH_INTERVAL_MS);
